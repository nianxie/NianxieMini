...
namespace TMPro
{
    ...
    public partial class TextMeshProUGUI
    {
        ...
        protected override void OnValidate()
        {
            ...
            if (realFontAsset == null || m_hasFontAssetChanged)
            {
                LoadFontAsset();
                m_hasFontAssetChanged = false;
            }

            if (m_canvasRenderer == null || m_canvasRenderer.GetMaterial() == null || m_canvasRenderer.GetMaterial().GetTexture(ShaderUtilities.ID_MainTex) == null || realFontAsset == null || realFontAsset.atlasTexture.GetInstanceID() != m_canvasRenderer.GetMaterial().GetTexture(ShaderUtilities.ID_MainTex).GetInstanceID())
            {
                LoadFontAsset();
                m_hasFontAssetChanged = false;
            }
        ...
        void ON_MATERIAL_PROPERTY_CHANGED(bool isChanged, Material mat)
        {
            ...
            if (m_shellFont != null)
            {
                if (m_shellFont.material == mat)
                {
                    m_shellFont.WriteBack();
                    UnityEditor.EditorUtility.SetDirty(m_shellFont);
                }
            }
            ...
            if (m_canvasRenderer == null || m_canvasRenderer.GetMaterial() == null)
            {
                ...
                if (realFontAsset != null)
                {
                    m_canvasRenderer.SetMaterial(realFontAsset.material, m_sharedMaterial.GetTexture(ShaderUtilities.ID_MainTex));
            ...
            if (m_canvasRenderer.GetMaterial() != m_sharedMaterial && realFontAsset == null) //    || m_renderer.sharedMaterials.Contains(mat))
            {
        ...
        protected override void LoadFontAsset()
        {
            ...
            {
                m_sharedMaterial = m_shellFont.material;
            }



            // Find and cache Underline & Ellipsis characters.
            GetSpecialCharacters(realFontAsset);
        ...
        internal override int SetArraySizes(UnicodeChar[] unicodeChars)
        {
            ...
            m_currentFontAsset = realFontAsset;
            ...
            */
            ...
            for (int i = 0; i < unicodeChars.Length && unicodeChars[i].unicode != 0; i++)
            {
                ...
                if (character == null)
                {
                    ...
                    if (!TMP_Settings.warningsDisabled)
                    {
                        string formattedWarning = srcGlyph > 0xFFFF
                            ? string.Format("The character with Unicode value \\U{0:X8} was not found in the [{1}] font asset or any potential fallbacks. It was replaced by Unicode character \\u{2:X4} in text object [{3}].", srcGlyph, realFontAsset.name, character.unicode, this.name)
                            : string.Format("The character with Unicode value \\u{0:X4} was not found in the [{1}] font asset or any potential fallbacks. It was replaced by Unicode character \\u{2:X4} in text object [{3}].", srcGlyph, realFontAsset.name, character.unicode, this.name);
                ...
                if (isUsingFallbackOrAlternativeTypeface && m_currentFontAsset.instanceID != realFontAsset.instanceID)
                {
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            if (realFontAsset == null || realFontAsset.characterLookupTable == null)
            {
            ...
            float baseScale = (m_fontSize / realFontAsset.m_FaceInfo.pointSize * realFontAsset.m_FaceInfo.scale * (m_isOrthographic ? 1 : 0.1f));
