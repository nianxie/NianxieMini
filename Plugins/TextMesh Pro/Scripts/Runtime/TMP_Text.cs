...
namespace TMPro
{
    ...
    public abstract class TMP_Text : MaskableGraphic
    {
        ...
        [SerializeField]
        protected TMPModify_ShellFont m_shellFont;
        [SerializeField] 
        protected TMP_FontAsset m_fontAsset;
        protected TMP_FontAsset realFontAsset => m_shellFont==null?m_fontAsset:m_shellFont.fontAsset;
        protected TMP_FontAsset m_currentFontAsset;
        protected bool m_isSDFShader;

        public void GetFont(out TMPModify_ShellFont shellFont, out TMP_FontAsset fontAsset)
        {
            shellFont = m_shellFont;
            fontAsset = realFontAsset;
        }

        public void SetFont(TMPModify_ShellFont shellFont, TMP_FontAsset fontAsset)
        {
            if(m_shellFont == shellFont && m_fontAsset == fontAsset)
            {
                return;
            }
            m_shellFont = shellFont;
        ...
        protected virtual Vector2 CalculatePreferredValues(ref float fontSize, Vector2 marginSize, bool isTextAutoSizingEnabled, bool isWordWrappingEnabled)
        {
            ...
            float baseScale = (fontSize / realFontAsset.faceInfo.pointSize * realFontAsset.faceInfo.scale * (m_isOrthographic ? 1 : 0.1f));
        ...
        protected virtual void DrawUnderlineMesh(Vector3 start, Vector3 end, ref int index, float startScale, float endScale, float maxScale, float sdfScale, Color32 underlineColor)
        {
            ...
            GetUnderlineSpecialCharacter(realFontAsset);
        ...
        internal TMP_TextElement GetTextElement(uint unicode, TMP_FontAsset fontAsset, FontStyles fontStyle, FontWeight fontWeight, out bool isUsingAlternativeTypeface)
        {
            ...
            if (fontAsset.instanceID != realFontAsset.instanceID)
            {
                // Search primary font asset
                character = TMP_FontAssetUtilities.GetCharacterFromFontAsset(unicode, realFontAsset, false, fontStyle, fontWeight, out isUsingAlternativeTypeface);
                ...
                if (realFontAsset.m_FallbackFontAssetTable != null && realFontAsset.m_FallbackFontAssetTable.Count > 0)
                    character = TMP_FontAssetUtilities.GetCharacterFromFontAssets(unicode, fontAsset, realFontAsset.m_FallbackFontAssetTable, true, fontStyle, fontWeight, out isUsingAlternativeTypeface);
                ...
                switch (m_xmlAttribute[0].nameHashCode)
                {
                    ...
                    case -842693512: // <LINE-HEIGHT>
                        ...
                        switch (tagUnitType)
                        {
                            ...
                            case TagUnitType.Percentage:
                                fontScale = (m_currentFontSize / m_currentFontAsset.faceInfo.pointSize * m_currentFontAsset.faceInfo.scale * (m_isOrthographic ? 1 : 0.1f));
                                m_lineHeight = realFontAsset.faceInfo.lineHeight * value / 100 * fontScale;
                                break;
                        }
                        return true;
