...
namespace TMPro.EditorUtilities
{
    public abstract class TMP_BaseEditorPanel : Editor
    {
        ...
        static readonly GUIContent k_ShellFontLabel = new GUIContent("Shell Font", "Font in shell app, will not be packed into AssetBundle.");
        ...
        protected SerializedProperty m_ShellFontProp;
        ...
        protected virtual void OnEnable()
        {
            ...
            m_ShellFontProp = serializedObject.FindProperty("m_shellFont");
        ...
        public void OnSceneGUI()
        {
            ...
            var fmh_342_63_638929757831391338 = Quaternion.identity; Vector3 newLeft = Handles.FreeMoveHandle(oldLeft, HandleUtility.GetHandleSize(m_RectTransform.position) * 0.05f, Vector3.zero, Handles.DotHandleCap);
            ...
            var fmh_357_61_638929757831418564 = Quaternion.identity; Vector3 newTop = Handles.FreeMoveHandle(oldTop, HandleUtility.GetHandleSize(m_RectTransform.position) * 0.05f, Vector3.zero, Handles.DotHandleCap);
            if (oldTop != newTop)
            {
            ...
            var fmh_371_65_638929757831422700 = Quaternion.identity; Vector3 newRight = Handles.FreeMoveHandle(oldRight, HandleUtility.GetHandleSize(m_RectTransform.position) * 0.05f, Vector3.zero, Handles.DotHandleCap);
            ...
            var fmh_385_67_638929757831426323 = Quaternion.identity; Vector3 newBottom = Handles.FreeMoveHandle(oldBottom, HandleUtility.GetHandleSize(m_RectTransform.position) * 0.05f, Vector3.zero, Handles.DotHandleCap);
        ...
        void DrawFont()
        {
            ...
            // FONT ASSET
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(m_ShellFontProp, k_ShellFontLabel);
            var shellFont = m_ShellFontProp.objectReferenceValue;
            if (shellFont == null)
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    Rect rect0 = EditorGUILayout.GetControlRect(false, 17);

                    EditorGUI.BeginProperty(rect0, k_MaterialPresetLabel, m_FontSharedMaterialProp);

                    float oldHeight = EditorStyles.popup.fixedHeight;
                    EditorStyles.popup.fixedHeight = rect0.height;
                    ...
                    m_MaterialPresetSelectionIndex = EditorGUI.Popup(rect0, k_MaterialPresetLabel, m_MaterialPresetSelectionIndex, m_MaterialPresetNames);
            ...
            {
                var shellFont0 = shellFont as TMPModify_ShellFont;
                if (m_FontAssetProp.objectReferenceValue != null)
                {
                    m_FontAssetProp.objectReferenceValue = null;
                }
                if (m_FontSharedMaterialProp.objectReferenceValue == null)
                {
                    m_FontSharedMaterialProp.objectReferenceValue = shellFont0.material;
                }
                if (EditorGUI.EndChangeCheck())
                {
                    m_HavePropertiesChanged = true;
                ...
                EditorGUILayout.ObjectField("Shell Font Asset", shellFont0.fontAsset, typeof(TMP_FontAsset), false);
                EditorGUILayout.PropertyField(m_FontSharedMaterialProp);
        ...
        {
            if (m_ShellFontProp.objectReferenceValue != null) return null;
