...
namespace Nianxie.Craft
{
    ...
    public class RectanglePacker {
	    public static Texture2D CreateTexture2DFromSprites(Sprite[] sprites)
	    {
            PackFromVec2s(sprites.Select(s => new Vector2Int((int)s.rect.width, (int)s.rect.height)).ToArray(), out var packRectArr, out var atlasSize);
            ...
            GL.PushMatrix();
            GL.LoadPixelMatrix(0, atlasSize.x, atlasSize.y, 0);

            // 遍历所有纹理并绘制到目标上
            for (int i = 0; i < sprites.Length; i++)
            {
                var sprite = sprites[i];
                var packRect = packRectArr[i];
